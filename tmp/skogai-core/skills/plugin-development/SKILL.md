...
version: 0.1.0
permalink: claude/tmp/skogai-core/skills/plugin-development/skill
---
...
1. Add frontmatter with `description` and optionally `allowed-tools`, `model`, `argument-hint`
1. Write the command body as instructions FOR Claude (not messages to the user)
...
1. Frontmatter must include: `name`, `description` (with `<example>` blocks), `model`, `color`
1. Body becomes the agent's system prompt (write in second person)
...
1. Frontmatter must include `name` and `description` with trigger phrases (third person)
1. Body uses imperative form, target 1,500-2,000 words
1. Move detailed content to `references/`, working code to `examples/`, utilities to `scripts/`
...
1. Add entry under the appropriate event
1. Choose `type: "command"` (deterministic) or `type: "prompt"` (LLM-driven)
1. Create scripts in `hooks/scripts/` for command hooks
...
1. Pick the right component type (command for user-invoked, agent for autonomous, skill for knowledge, hook for automation)
1. Create the file in the right directory
1. Follow the format for that component type
1. Restart Claude Code to pick up changes
1. Test with `claude --debug` for hooks, trigger phrases for skills, `/command` for commands
