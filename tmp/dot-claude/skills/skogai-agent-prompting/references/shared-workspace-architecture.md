---
title: shared-workspace-architecture
type: note
permalink: claude/tmp/dot-claude/skills/skogai-agent-prompting/references/shared-workspace-architecture
---
...
1. User starts research agent
1. Agent downloads full text to `Research/book_moby_dick/full_text.txt`
1. Agent researches and writes to `sources/`
1. Agent generates `introduction.md` based on user's reading profile
1. User can view all files in the app or Files.app
1. User can edit `introduction.md` to refine it
1. Chat agent can read all of this context when answering questions </examples>
...
1. **User opens app on iPad** → Same experiment visible, no sync code needed
1. **Agent logs observation on iPhone** → Syncs to iPad automatically
1. **User edits journal on iPad** → iPhone sees the edit
...
- [ ] Use NSFileCoordinator for conflict-safe writes </checklist>
