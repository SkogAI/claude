...
title: verify-skill
type: note
permalink: claude/tmp/dot-claude/skills/skogai-routing/workflows/verify-skill
---
...
Skills contain claims about external things: APIs, CLI tools, frameworks, services. These change over time. This workflow checks if a skill's content is still accurate. </purpose>
...
1. **Review each** - Show each change before applying
1. **Just the report** - No changes
...
"This skill should be re-verified in approximately [timeframe]." </process>
...
- Current best practices: "{framework} best practices 2025" \</verification_shortcuts>

\<success_criteria> Verification is complete when:
...
- [ ] User knows when to re-verify \</success_criteria>
