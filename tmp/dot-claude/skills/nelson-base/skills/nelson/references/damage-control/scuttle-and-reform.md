---
title: scuttle-and-reform
type: note
permalink: claude/tmp/dot-claude/skills/nelson-base/skills/nelson/references/damage-control/scuttle-and-reform
---
...
1. Each agent saves current partial outputs and documents their last known state.
1. Admiral produces an abort log using the Captain's Log Template with:
   - Reason for abort.
...
1. Admiral issues shutdown requests to all agents.
1. Admiral presents the abort log to the human (Admiralty) with a recommendation: retry with new constraints, descope, or abandon.
