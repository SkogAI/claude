...
permalink: claude/tmp/dot-claude/commands/wrap-cli-as-api
---
...
1. **Create a systemd start script** at `~/.config/systemd/<service-name>-start.sh`
...
1. **Create a systemd user service** at `~/.config/systemd/user/<service-name>.service`
...
1. **Test each endpoint** with `curl` and verify JSON output
