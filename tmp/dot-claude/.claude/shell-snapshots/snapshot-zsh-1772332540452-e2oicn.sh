...
EOH
	...
	[[ -n "$name" ]] || {
		gvim || return $?
		while name=$(gvim --serverlist)  && [[ -z "$name" ]]
		do
			sleep 0.1
		done
	}
...
layout_uv () {
	...
	then
		...
		then
			...
			noglob uv venv "$UV_PROJECT_ENVIRONMENT"
		else
			noglob uv venv
...
rbwpw () {
	...
	{
		sleep 20 && [[ "$(<"$_cache")" == "$_random" ]] && clipcopy < /dev/null 2> /dev/null && command rm -f "$_cache" &> /dev/null
	} &|
}
regexp-replace () {
	argv=("$1" "$2" "$3") 
...
systemd_prompt_info () {
	...
	do
		...
		if [[ -n "$ZSH_THEME_SYSTEMD_PROMPT_CAPS" ]]
		then
			echo -n "${(U)unit:gs/%/%%}:"
		else
			echo -n "${unit:gs/%/%%}:"
		...
		elif systemctl --user is-active "$unit" &> /dev/null
		then
			echo -n "$ZSH_THEME_SYSTEMD_PROMPT_ACTIVE"
		else
...
alias -- commit='aider --commit --model openrouter/openrouter/free --no-gitignore --set-env OPENROUTER_API_KEY=$OPENROUTER_API_KEY'
...
if ! (unalias rg 2>/dev/null; command -v rg) >/dev/null 2>&1; then
  alias rg='/opt/claude-code/bin/claude --ripgrep'
fi
export PATH=/home/skogix/.local/src/argc-completions/bin\:/home/skogix/.local/src/argc-completions/bin\:/home/skogix/.nvm/versions/node/v24.14.0/bin\:/home/skogix/.cargo/bin\:/mnt/sda2/go/bin\:/home/skogix/bin\:/home/skogix/.local/bin\:/home/skogix/.cargo/bin\:/mnt/sda2/go/bin\:/home/skogix/bin\:/home/skogix/.local/bin\:/usr/local/bin\:/usr/bin\:/bin\:/usr/local/sbin\:/usr/share/dotnet\:/home/skogix/.dotnet/tools\:/usr/bin/site_perl\:/usr/bin/vendor_perl\:/usr/bin/core_perl
