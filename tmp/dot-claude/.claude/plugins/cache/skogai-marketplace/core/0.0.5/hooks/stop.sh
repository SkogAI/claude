#!/usr/bin/env bash
# Stop hook - runs when main Claude Code agent finishes responding
#
...
#   "stop_hook_active": true  # True when Claude is continuing due to stop hook
# }
#
# Note: Check stop_hook_active to prevent infinite loops. When true, Claude is
# already continuing as a result of a stop hook.
#
# Output JSON structure (optional):
# {
#   "decision": "block" | undefined,  # "block" prevents Claude from stopping
#   "reason": "string"  # Must be provided when blocking - tells Claude how to proceed
# }
#
# Exit codes:
# - 0: Success. JSON output parsed for decision control
# - 2: Blocks stoppage, shows stderr to Claude
# - Other: Non-blocking error, stderr shown to user in verbose mode

input=$(cat)
session_id=$(echo "$input" | jq -r '.session_id')
log_file="/tmp/${session_id}-stop.json"
echo "$input" >"$log_file"
...
# Parse last line of claude's response through skogparse
