...
2. Updates `marketplace.json` with the new version
3. Updates `changelog.md` with commit history
4. Creates a git commit with auto-generated message
5. Creates a git tag in format `core/vX.Y.Z`
6. Pushes commit and tag to remote (unless `--no-push`)
7. Refreshes marketplace cache (unless `--no-refresh`)
8. Installs the updated plugin
...
2. **UserPromptSubmit** (`user-prompt-submit.sh`): Parses skogai notation (e.g., `$ref.path`, `[@action]`)
3. **PreToolUse** (`pre-tool-use.sh`): Can add context or block tool calls before execution
4. **PostToolUse** (`post-tool-use.sh`): Reacts to tool execution results
5. **PreCompact** (`pre-compact.sh`): Runs before context compaction
6. **Stop** (`stop.sh`): Session termination handler
7. **SubagentStop** (`subagent-stop.sh`): Subagent termination handler
8. **Notification** (`notification.sh`): Handles notifications
...
2. Test changes locally (plugin cache auto-refreshes on file changes via symlinks)
3. Bump version: `/argc bump core [patch|minor|major]`
4. The bump command handles: version update, changelog, git commit/tag, push, refresh, install
...
2. Changes are immediately available (symlinks + cache auto-update)
3. Invoke updated commands/agents to verify behavior
4. When satisfied, run version bump workflow
