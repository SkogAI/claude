---
title: rosy-whistling-horizon
type: note
permalink: claude/tmp/dot-claude/plans/rosy-whistling-horizon
---
...
1. **Human CLI**: `skog argc run searxng --action up` → `Argcfile.sh run()` → `bash scripts/searxng.sh --action up`

1. **AI agent tool**: `./utils/run-script.sh searxng '{"action":"up"}'` → jq converts JSON to flags → `./scripts/searxng.sh --action up`

Both paths converge on the same call: `./scripts/searxng.sh --flag value`. This is the "gain for free" — argc annotations become the schema for both humans and AI agents.

The current scripts are wrong: they use `@cmd main` + `@arg action` (positional). `run-script.sh` always emits `--key value` flags from JSON — positional args are unreachable. Scripts must use `@describe` + `@option` instead.
...
Currently checks for `grep -q '^main()'`. With `@describe`, main() still exists, so this check still works. No change needed.
...
The actual `main()` implementations (podman run, start, stop, etc.) and the `services/manifest.toml` + `lib/container.sh` architecture are separate work. This plan only fixes the interface layer so both invocation paths work correctly.
