---
title: ticklish-yawning-coral
type: note
permalink: claude/tmp/dot-claude/plans/ticklish-yawning-coral
---
...
1. Create `~/.config/dolt/dbs/` data directory
1. Deploy systemd user service from template
1. Enable and start the service (`systemctl --user enable --now dolt-server`)
1. Initialize dolt global config (user.name, user.email)
1. Add DoltHub remote for `skogix/claude` if not present
...
1. `mysql -h 127.0.0.1 -P 3306 -u root -e "SHOW DATABASES"` — lists skogix/claude db
1. `dolt log` in `~/.config/dolt/dbs/skogix/claude/` — shows commit history
1. Reboot test: service comes back automatically
1. gt/beads unaffected: `bd dolt test` still works on port 3307
