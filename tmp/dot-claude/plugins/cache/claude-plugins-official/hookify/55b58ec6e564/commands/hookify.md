...
allowed-tools:
  ...
  - TodoWrite
  - Skill
permalink: claude/tmp/dot-claude/plugins/cache/claude-plugins-official/hookify/55b58ec6e564/commands/hookify
---
...
1. Use Write tool to create each `.claude/hookify.{name}.local.md` file
...
1. Ask: "Should I block this command or just warn you?"
...
1. Ensure `.claude/` directory exists (create with mkdir if needed)
1. Use absolute path if needed: `{cwd}/.claude/hookify.{name}.local.md`
1. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**

1. Verify file is in project `.claude/` not plugin `.claude/`
1. Check file with Read tool to ensure pattern is correct
1. Test pattern with: `python3 -c "import re; print(re.search(r'pattern', 'test text'))"`
1. Verify `enabled: true` in frontmatter
1. Remember: Rules work immediately, no restart needed

**If blocking seems too strict:**

1. Change `action: block` to `action: warn` in the rule file
1. Or adjust the pattern to be more specific
1. Changes take effect on next tool use
