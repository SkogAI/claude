...
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/skills/memory-systems/skill
---
...
1. **Scale**: Move to Mem0 or vector store with metadata when you need semantic search and multi-tenant isolation.
...
1. **Full control**: Use Letta or Cognee when you need agent self-management of memory with deep introspection.
...
1. Track temporal validity for any fact that can change over time
1. Use hybrid retrieval (semantic + keyword + graph) for best accuracy
1. Consolidate memories periodically — invalidate but don't discard
1. Design for retrieval failure: always have a fallback when memory lookup returns nothing
1. Consider privacy implications of persistent memory (retention policies, deletion rights)
1. Benchmark your memory system against LoCoMo or LongMemEval before and after changes
1. Monitor memory growth and retrieval latency in production
