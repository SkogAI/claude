...
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/skills/project-development/skill
---
...
1. **Prepare**: Transform data into prompt format
1. **Process**: Execute LLM calls (the expensive, non-deterministic step)
1. **Parse**: Extract structured data from LLM outputs
1. **Render**: Generate final outputs (reports, files, visualizations)
...
1. **Format examples**: Show exactly what output should look like
1. **Rationale disclosure**: "I will be parsing this programmatically"
1. **Constrained values**: Enumerated options, score ranges, formats
...
1. Let the agent generate initial implementation
1. Test and iterate on specific failures
1. Refine prompts and architecture based on results
...
1. Structure pipelines as discrete, idempotent, cacheable stages
1. Use the file system for state management and debugging
1. Design prompts for structured, parseable outputs with explicit format examples
1. Start with minimal architecture; add complexity only when proven necessary
1. Estimate costs early and track throughout development
1. Build robust parsers that handle LLM output variations
1. Expect and plan for multiple architectural iterations
1. Test whether scaffolding helps or constrains model performance
1. Use agent-assisted development for rapid iteration on implementation
