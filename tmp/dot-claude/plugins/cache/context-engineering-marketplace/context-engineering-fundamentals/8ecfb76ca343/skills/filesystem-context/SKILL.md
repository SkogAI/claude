...
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/skills/filesystem-context/skill
---
...
1. Store plans and state in structured files for re-reading
1. Use sub-agent file workspaces instead of message chains
1. Load skills dynamically rather than stuffing all into system prompt
1. Persist terminal and log output as searchable files
1. Combine grep/glob with semantic search for comprehensive discovery
1. Organize files for agent discoverability with clear naming
1. Measure token savings to validate filesystem patterns are effective
1. Implement cleanup for scratch files to prevent unbounded growth
1. Guard self-modification patterns with validation
