...
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/skills/context-compression/skill
---
...
1. **Opaque Compression**: Produce compressed representations optimized for reconstruction fidelity. Achieves highest compression ratios (99%+) but sacrifices interpretability. Cannot verify what was preserved.

1. **Regenerative Full Summary**: Generate detailed structured summaries on each compression. Produces readable output but may lose details across repeated compression cycles due to full regeneration rather than incremental merging.
...
1. **Context Awareness**: Does the response reflect current conversation state?
1. **Artifact Trail**: Does the agent know which files were read or modified?
1. **Completeness**: Does the response address all parts of the question?
1. **Continuity**: Can work continue without re-fetching information?
1. **Instruction Following**: Does the response respect stated constraints?
...
1. **Planning Phase**: Convert research into implementation specification with function signatures, type definitions, and data flow. A 5M token codebase compresses to approximately 2,000 words of specification.

1. **Implementation Phase**: Execute against the specification. Context remains focused on the spec rather than raw codebase exploration.
...
1. On first compression trigger, summarize truncated history into sections
1. On subsequent compressions, summarize only new truncated content
1. Merge new summary into existing sections rather than regenerating
1. Track which information came from which compression cycle for debugging
...
1. Use structured summaries with explicit sections for file tracking
1. Trigger compression at 70-80% context utilization
1. Implement incremental merging rather than full regeneration
1. Test compression quality with probe-based evaluation
1. Track artifact trail separately if file tracking is critical
1. Accept slightly lower compression ratios for better quality retention
1. Monitor re-fetching frequency as a compression quality signal
