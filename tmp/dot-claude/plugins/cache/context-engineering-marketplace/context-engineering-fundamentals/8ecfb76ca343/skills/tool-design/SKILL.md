...
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/skills/tool-design/skill
---
...
1. Standard tools have predictable, well-documented behavior
1. The agent can chain primitives flexibly rather than being constrained to predefined workflows
1. Good documentation in files replaces the need for summarization tools
...
1. Group related actions into comprehensive tools
1. Ensure each tool has a clear, unambiguous purpose
1. Document error cases and recovery paths
...
1. **Missing parameters**: What database? What format should query take?
1. **No return description**: What does this function return? A list? A string? Error handling?
1. **No usage context**: When should an agent use this versus other tools?
1. **No error handling**: What happens if the database is unavailable?
...
1. Use consolidation to reduce ambiguity
1. Implement response format options for token efficiency
1. Design error messages for agent recovery
1. Establish and follow consistent naming conventions
1. Limit tool count and use namespacing for organization
1. Test tool designs with actual agent interactions
1. Iterate based on observed failure modes
1. Question whether each tool enables or constrains the model
1. Prefer primitive, general-purpose tools over specialized wrappers
1. Invest in documentation quality over tooling sophistication
1. Build minimal architectures that benefit from model improvements
