...
title: case-studies
type: note
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/skills/project-development/references/case-studies
---
...
1. **File system as state**: Each article directory contains all intermediate outputs, making debugging trivial.

1. **Idempotent stages**: Re-running only processes items that lack output files.

1. **Agent-assisted development**: 3 hours to working code by focusing on requirements, not implementation details.

1. **Parallel execution**: 15 workers reduced execution time without increasing token costs.
...
1. **File systems are proven abstractions**: The model understands file systems deeply from training. grep is 50 years old and works perfectly.

1. **Constraints became liabilities**: With better models, the guardrails were limiting performance more than helping.
...
1. **Build for future models**: Models improve faster than tooling. Architectures optimized for today may be over-constrained for tomorrow.

1. **Good context over clever tools**: Invest in documentation, clear naming, and well-structured data. That foundation matters more than sophisticated tooling.

1. **Start simple**: Model + file system + goal. Add complexity only when proven necessary.
...
1. **Teach delegation**: Subagents need objective, output format, tools/sources guidance, and clear boundaries.

1. **Scale effort to complexity**: Explicit guidelines for agent/tool call counts by task type.

1. **Tool design is critical**: Distinct purpose and clear description for each tool. Bad descriptions send agents down wrong paths entirely.

1. **Let agents improve themselves**: Claude 4 models can diagnose prompt failures and suggest improvements. Tool-testing agents can rewrite tool descriptions to avoid common mistakes.

1. **Start wide, then narrow**: Broad queries first, evaluate landscape, then drill into specifics.

1. **Guide thinking process**: Extended thinking mode as controllable scratchpad for planning.

1. **Parallel tool calling**: 3-5 subagents in parallel, 3+ tools per subagent in parallel. Cut research time by up to 90%.
...
1. **File system as foundation**: Whether for state management (Karpathy), tool interface (Vercel), or memory (Manus), the file system provides proven abstractions.

1. **Architectural simplicity**: Reduction outperformed complexity in multiple cases. Start minimal, add only what proves necessary.

1. **Structured outputs with robust parsing**: Explicit format specifications combined with flexible parsing that handles variations.

1. **Iteration expectation**: No project got architecture right on the first try. Build for change.

### Common Failure Patterns

1. **Over-constraining models**: Guardrails that helped with weaker models become liabilities as capabilities improve.

1. **Tool proliferation**: More tools often means more confusion and worse performance.

1. **Hiding errors**: Removing failures from context prevents models from learning.

1. **Premature optimization**: Adding complexity before basic functionality works.

1. **Ignoring economics**: Token costs compound quickly; estimation and tracking are essential.
