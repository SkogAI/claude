...
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/skills/hosted-agents/skill
---
...
1. Sandbox sends event to API with branch name and session ID
1. API uses user's GitHub token to create PR
1. GitHub webhooks notify API of PR events
...
1. Start warming sandboxes when users begin typing, not when they submit
1. Allow file reads before git sync completes; block only writes
1. Structure agent framework as server-first with clients as thin wrappers
1. Isolate state per session to prevent cross-session interference
1. Attribute commits to the user who prompted, not the app
1. Track merged PRs as primary success metric
1. Build for multiplayer from the start; it is nearly free with proper sync architecture
