---
title: architectural_reduction
type: note
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/skills/tool-design/references/architectural-reduction
---
...
1. **Model capability is sufficient**: The model can reason through complexity without hand-holding.

1. **Safety constraints permit**: The sandbox limits what the agent can access and modify.

1. **Domain is navigable**: The problem space can be explored through file inspection.
...
1. **Specialized knowledge is required**: Domain expertise that can't be documented in files.

1. **Safety requires restrictions**: Operations that must be constrained for security or compliance.

1. **Workflows are genuinely complex**: Multi-step processes that benefit from structured orchestration.
...
1. **Failure analysis**: Are failures caused by model limitations or tool constraints?

1. **Documentation quality**: Could the model navigate your data layer directly if given access?

1. **Constraint necessity**: Are guardrails protecting against real risks or hypothetical concerns?

1. **Model capability**: Has the model improved since tools were designed?
