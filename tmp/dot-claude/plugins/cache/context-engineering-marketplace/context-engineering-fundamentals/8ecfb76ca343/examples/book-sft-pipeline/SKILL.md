...
version: 2.0.0
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/examples/book-sft-pipeline/skill
---
...
1. **Use paragraph-level extraction** - Extract from `<p>` tags to preserve breaks
1. **Remove front/back matter** - Copyright and TOC pollute the dataset
...
1. **Always source ePub over PDF** - OCR errors become learned patterns
1. **Never break mid-sentence** - Boundaries must be grammatically complete
1. **Use diverse prompts** - 15+ templates, 5+ system prompts
1. **Use base models** - Not instruct versions
1. **Use smaller chunks** - 150-400 words for more examples
1. **Reserve test set** - 50 examples minimum
1. **Test on modern scenarios** - Proves style transfer vs memorization
1. **Verify originality** - Grep training data for output phrases
