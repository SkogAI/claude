...
version: 1.0.0
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/examples/digital-brain-skill/skill
---
...
1. Check `identity/brand.md` → Confirm "ai_agents" is a content pillar
1. Reference `content/posts.jsonl` → Find similar successful posts
1. Draft post matching voice patterns
1. Suggest adding to `content/ideas.jsonl` if not publishing immediately
...
1. Get recent entries from `network/interactions.jsonl`
1. Check `operations/todos.md` for pending items with Sarah
1. Compile brief: role, context, last discussed, follow-ups
...
1. **Append Only**: Never delete from JSONL files - archive instead
1. **Update Timestamps**: Set `updated` field when modifying tracked data
1. **Cross-Reference**: Knowledge informs content, network informs operations
1. **Log Interactions**: Always log meetings/calls to `interactions.jsonl`
1. **Preserve History**: Past content in `posts.jsonl` informs future performance
