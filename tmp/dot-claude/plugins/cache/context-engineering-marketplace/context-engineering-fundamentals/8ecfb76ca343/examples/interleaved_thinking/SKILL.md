...
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/examples/interleaved-thinking/skill
---
...
1. **External perturbations** (tool outputs, environment changes) need real-time adaptation
1. **Debugging** requires seeing HOW decisions were made, not just WHAT was output
...
1. Optionally update the system prompt

## Guidelines

1. **Preserve full context**: M2.1 requires full response history including thinking blocks for optimal performance
1. **Use appropriate tools**: Define tools clearly with unambiguous descriptions
1. **Set realistic convergence thresholds**: 5-10% improvement per iteration is typical
1. **Review generated skills**: Auto-generated skills should be reviewed before sharing
1. **Monitor token usage**: Each optimization iteration uses significant tokens
