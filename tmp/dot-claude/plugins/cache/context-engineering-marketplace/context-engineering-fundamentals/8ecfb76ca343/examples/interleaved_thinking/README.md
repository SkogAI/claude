...
title: README
type: note
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/examples/interleaved-thinking/readme
---
...
1. **Pattern Detection** - Automatically identify failure modes (context degradation, tool confusion, etc.)
1. **Automated Optimization** - Generate improved prompts based on detected issues
1. **Shareable Skills** - Convert learnings into reusable Agent Skills for team sharing
...
1. **Tool Calls** - What tools were called with what inputs
1. **Tool Results** - What each tool returned
1. **Final Response** - The agent's output
1. **Metadata** - Tokens used, turns taken, success/failure
...
1. **Best Score Tracking** - Automatically uses the best-performing prompt, even if later iterations regress
1. **Regression Detection** - Warns when scores drop and can stop after consecutive regressions
