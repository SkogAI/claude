...
type: note
permalink: claude/tmp/dot-claude/plugins/cache/context-engineering-marketplace/context-engineering-fundamentals/8ecfb76ca343/researcher/example-output
---

# The Infinite Software Crisis – Jake Nations, Netflix

**Evaluation ID:** `a7f3c8e1-4b2d-4f9a-8c1e-3d5f7a9b2c4e` **Timestamp:** 2025-01-10T14:32:00Z **Source:**
...
**Weighted Total:** `1.45` *Calculation:* `(1×0.35) + (2×0.30) + (1×0.20) + (2×0.15) = 0.35 + 0.60 + 0.20 + 0.30 = 1.45`
...
1. **ADD** 'earned understanding' principle: The insight that manual migration should precede AI-assisted work—do one task by hand, then use that artifact as research seed.
1. **ADD** to 'When to Activate': Recognition that context compression is needed when codebases exceed context windows (5M tokens example).
1. **CONSIDER** new skill: 'Easy vs Simple for AI' could be a standalone skill about recognizing when AI tooling encourages complexity accumulation vs. structural simplicity.
1. **VERIFY**: The Netflix claims are reasonable but unquantified. Before citing, confirm the three-phase approach produces measurable improvements in the authorization refactor case.
