...
title: original-plan
type: note
permalink: claude/tmp/claude-code/original-plan
---
...
1. `cd ~/projects/system` — confirm `SKOGAI.md` now has installed software section
1. `cd ~/projects/git` — confirm `SKOGAI.md` exists with identity/auth info
1. Check no content duplication: system specs appear only in `projects/system/SKOGAI.md`, not in `~/CLAUDE.md`
