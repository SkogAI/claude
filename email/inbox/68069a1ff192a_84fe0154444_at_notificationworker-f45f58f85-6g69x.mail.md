MIME-Version: 1.0
From: GitHub <noreply@github.com>
To: emil.skogsund@gmail.com
Date: Mon, 21 Apr 2025 12:18:55 -0700
Subject: [GitHub] Subscribed to verificay/SkogDATA notifications
Message-ID: <68069a1ff192a_84fe0154444@notificationworker-f45f58f85-6g69x.mail>
Content-Type: text/plain; charset="UTF-8"

Hey there, we’re just writing to let you know that you’ve been automatically subscribed to a repository on GitHub.

    verificay/SkogDATA
    SkogAI Data
    https://github.com/verificay/SkogDATA

You’ll receive notifications for all issues, pull requests, and comments that happen inside the repository. If you would like to stop watching this repository, you can manage your settings here:

    https://github.com/verificay/SkogDATA/subscription

You can unwatch this repository immediately by clicking here:

    https://github.com/verificay/SkogDATA/unsubscribe_via_email/AAFIQZ3X5U5QARM6MUUGDFL22VAB7ANCNFSM4OOVLJ6A

You were automatically subscribed because you’ve been given access to the repository.

Thanks!
