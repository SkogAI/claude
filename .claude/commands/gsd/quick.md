...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" frontmatter get .planning/quick/{dir}/SUMMARY.md --field status 2>/dev/null
  ```
...
5. Load context via:
   ```bash
   node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" init quick
   ```
