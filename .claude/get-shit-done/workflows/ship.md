...
```bash
BASE_BRANCH=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get git.base_branch 2>/dev/null || echo "")
...
```bash
REVIEW_CMD=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_command --default "" 2>/dev/null)
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" state load 2>/dev/null | head -20)
   ```
