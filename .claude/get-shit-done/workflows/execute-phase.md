...
@/home/skogix/claude/.claude/get-shit-done/references/agent-contracts.md
@/home/skogix/claude/.claude/get-shit-done/references/context-budget.md
@/home/skogix/claude/.claude/get-shit-done/references/gates.md
</required_reading>
...
AGENT_SKILLS=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
CONTEXT_WINDOW=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get context_window 2>/dev/null || echo "200000")
```
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/home/skogix/claude/.claude/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/home/skogix/claude/.claude/get-shit-done/references/planner-antipatterns.md
...
```bash
CROSS_AI_ENABLED=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.cross_ai_execution --default false 2>/dev/null)
CROSS_AI_CMD=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.cross_ai_command --default "" 2>/dev/null)
CROSS_AI_TIMEOUT=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.cross_ai_timeout --default 300 2>/dev/null)
```
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       ${CONTEXT_WINDOW < 200000 ? '' : '@/home/skogix/claude/.claude/get-shit-done/references/executor-examples.md'}
       </execution_context>
   ...
   for WT in $WORKTREES; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       if ! git diff --quiet .planning/STATE.md .planning/ROADMAP.md 2>/dev/null || \
         ...
         COMMIT_DOCS=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get commit_docs 2>/dev/null || echo "true")
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
     ...
     if ! git diff --quiet .planning/ROADMAP.md .planning/STATE.md 2>/dev/null; then
       node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" --files .planning/ROADMAP.md .planning/STATE.md
     fi
   elif [ "${TEST_EXIT}" -eq 124 ]; then
...
**Security gate check:**
```bash
SECURITY_CFG=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
...
```bash
TDD_MODE=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.tdd_mode --default false 2>/dev/null)
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review 2>/dev/null || echo "true")
```
...
```bash
SCHEMA_DRIFT=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
```bash
VERIFIER_SKILLS=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-verifier 2>/dev/null)
```
...
**Check config gate:**
```bash
GL_ENABLED=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" learnings copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
